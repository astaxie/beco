...
func (r *Router) Add(method, path string, h http.HandlerFunc) {
	...
	for i, l := 0, len(path); i < l; i++ {
		if path[i] == ':' {
			...
			if i == l {
				r.insert(method, path[:i], h, pkind, pnames)
				return
			}
			r.insert(method, path[:i], nil, pkind, pnames)
		} else if path[i] == '*' {
			r.insert(method, path[:i], nil, skind, nil)
			pnames = append(pnames, "_*")
			r.insert(method, path[:i+1], h, mkind, pnames)
			return
		}
	}

	r.insert(method, path, h, skind, pnames)
}

func (r *Router) insert(method, path string, h http.HandlerFunc, t kind, pnames []string) {
...
func (r *Router) Find(method, path string) (h HandlerFunc, params []string) {
	h = notFoundHandler
	e = r.echo
	cn := r.tree // Current node as root
